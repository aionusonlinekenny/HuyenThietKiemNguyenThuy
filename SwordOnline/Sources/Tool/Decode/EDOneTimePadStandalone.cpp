...
void EDOneTimePad_Encipher(char* pPlaintext, int nPlainLen)
{
    if (pPlaintext && nPlainLen > 0)
    {
        ...
        {
            pContent[nPos] = (unsigned char)((pContent[nPos] - 0x20 + (0xff - cCipher[cCipher[0] % 3])) % 0xdf + 0x20);
            cCipher[0] = cCipher[1];
            cCipher[1] = cCipher[2];
...
void EDOneTimePad_Decipher(char* pCiphertext, int nCiphertextLen)
{
    if (pCiphertext && nCiphertextLen > 0)
    {
        ...
        for (nPos = nCiphertextLen - 1; nPos >= NUM_FIX_CIPHER_CHARA; nPos--)
            pContent[nPos] = (unsigned char)((pContent[nPos] - 0x20 + (pContent[nPos - 3 + (pContent[nPos - 3] % 3)] - 0x20)) % 0xdf + 0x20);
        ...
        for (; nPos >= 0; nPos--)
        {
            pContent[nPos] = (unsigned char)((pContent[nPos] - 0x20 + (cCipher[nPos + (cCipher[nPos] % 3)] - 0x20)) % 0xdf + 0x20);
        }
    }
}
