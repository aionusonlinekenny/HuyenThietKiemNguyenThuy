...
void CD3D_Device::ListDeviceCaps()
{
	...
	//g_DebugLog("[D3DRender]VendorID: 0x%x, DeviceID: 0x%x, SubSysID: 0x%x, Revision: 0x%x", m_pAdapter->AdapterID.VendorId, m_pAdapter->AdapterID.DeviceId, m_pAdapter->AdapterID.SubSysId, m_pAdapter->AdapterID.Revision);
	//g_DebugLog("[D3DRender]Width: %d, Height: %d, BitDepth: %d", m_pMode->Width,m_pMode->Height,iBitCount);
	//g_DebugLog("[D3DRender]---"); 
}
