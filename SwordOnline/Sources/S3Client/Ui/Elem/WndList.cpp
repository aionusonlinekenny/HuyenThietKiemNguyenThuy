...
void KWndList::Clone(KWndList* pCopy)
{
	if (pCopy)
	{
		...
		strcpy(pCopy->m_szImage, m_szImage);
		pCopy->m_nImgWidth = m_nImgWidth;
		pCopy->m_nImgHeight = m_nImgHeight;
		pCopy->m_nItemTextInent = m_nItemTextInent;
...
int KWndList::Init(KIniFile* pIniFile, const char* pSection)
{
	if (KWndWindow::Init(pIniFile, pSection))
	{
		...
		if (m_nNumColumn < 1)
			m_nNumColumn = 1;
		pIniFile->GetShort(pSection, "RowSpace", 0, &m_nRowSpace);
		...
		pIniFile->GetShort(pSection, "ItemTextInent", 0, &m_nItemTextInent);
...
{	
	...
	if(m_szImage[0])
	{
		nDef = m_nImgHeight + m_nRowSpace / 2 - m_nNumColumn;
	}
	else
	{
		nDef = m_nFontSize + 1;
	...
	int nSel = m_nTopItemIndex + (y - m_nAbsoluteTop) / nDef * m_nNumColumn + m_nRowDis;
	if (m_nNumColumn > 1)
	{
...
void KWndList::PaintWindow()
{
	...
	if (g_pRepresentShell)
	{
		...
		int	nMaxLen = (nColumnWidth * 2) / m_nFontSize + 1 + m_nRowDis;
		...
		{	
			if (nColumn == m_nNumColumn)
			{
				...
				if(m_szImage[0])
				{
					y += m_nImgHeight + m_nRowSpace / 2 - m_nNumColumn;
				}
				else
				{
			...
			{				
				...
				{	
					...
					Image.nFrame = 1;
					Image.oPosition.nX = x;
					Image.oPosition.nY = y;
			...
			if(m_szImage[0])
			{ 
				nPaintX += m_nItemTextInent * m_nFontSize / 2;
			}

			nPaintY = y;
			if(m_szImage[0])
			{
				nPaintY += m_nItemTextInent / 2 + 1; 
			}

			g_pRepresentShell->OutputText(m_nFontSize, pShowString, nLen, nPaintX, nPaintY,
...
{
	return (m_Height / (m_nFontSize + 1 + m_nRowDis)) * m_nNumColumn;
}

void KWndList::SetImage(const char* pszImage, short nWidth, short nHeight)
{
	if(!pszImage || !pszImage[0])
		return;
	
	strcpy(m_szImage, pszImage);
	m_nImgWidth		= nWidth;
