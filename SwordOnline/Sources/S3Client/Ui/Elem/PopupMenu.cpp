...
void KPopupMenu::LoadTheme(const char* pScheme)
{
	...
	{
		int nValue;
		Ini.GetInteger("MouseOverWnd", "ImgWidth",  0, &nValue);
		if(nValue >= 0)
			m_nImgWidth = nValue;
		else
			m_nImgWidth = 0;
		Ini.GetInteger("MouseOverWnd", "ImgHeight", 0, &nValue);
		if(nValue >= 0)
			m_nImgHeight = nValue;
		else
			m_nImgHeight = 0;
		Ini.GetInteger("MouseOverWnd", "Indent",    0, &nValue);
		m_nIndent = nValue;
		Ini.GetInteger("MouseOverWnd", "Frame", 0, &nValue);
		m_nFrame = nValue;
		Ini.GetString("MouseOverWnd", "Image", "" , m_szImage, sizeof(m_szImage));
