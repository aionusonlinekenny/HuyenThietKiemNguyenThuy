...
//
// --
int KWndMessageListBox::AddOneMessage(const char* pText, int nLen, const char* pszName /*= NULL*/, const char* pszInfo /*= NULL*/, 
									  int nPos /*= 0*/, int nLinePos /*= 1*/, int nFace /*= 0*/, unsigned int uTextBKColor)
{
	if (pText == NULL)
		return 0;
	if (nLen < 0)
	...
	if (m_nCapability > 0 && nLen)
	{
		...
		{
			ZeroMemory(pMsg->szName, sizeof(pMsg->szName));
			ZeroMemory(pMsg->szInfo, sizeof(pMsg->szInfo));
			pMsg->nPos = 0;
			pMsg->nLinePos = 0;
			pMsg->nFace = 0;
		}
...
void KWndMessageListBox::PaintWindow()
{
	...
	if (m_nHideNumLine >= m_nNumVisibleTextLine)
		return;
	...
	if(nCurMsg && m_bItemActived)
	{
		for(int i = nCurMsg - 1; i >= 0; i--)
		{
			if(m_pMessages[i]) 
			{
				KUiPlayerBar::SetChatItemPosition(i, defHIDE_CHATITEM_BUTTON, false);
			}
		}
	}
	...
	{	
		...
		if (bDraw)
		{
			...
			if(m_bItemActived)
			{
				...
				{
					KUiPlayerBar::SetChatItemPosition(nCurMsg,Param.nX + (m_pMessages[nCurMsg]->nPos) * m_nFontSize / 2 + (m_pMessages[nCurMsg]->nFace) * 2,
												Param.nY + ((m_pMessages[nCurMsg]->nLinePos)-1 - Param.nSkipLine) * m_nFontSize, true);
				}
				else 
				{
					KUiPlayerBar::SetChatItemPosition(nCurMsg, defHIDE_CHATITEM_BUTTON, false);
				}
			}
...
void KWndMessageListBox::UpdateChatItem()
{
	if(m_bItemActived)
	{
		...
		for (int i = 0; i < m_nCapability; i++)
		{
			if (m_pMessages[i])
			{	
				KUiPlayerBar::SetChatItemInfo(i, m_pMessages[i]->szName, m_pMessages[i]->szInfo);
			}
		}
	}
}
...
int KScrollMessageListBox::Init(KIniFile* pIniFile, const char* pSection)//³õÊ¼»¯
{
	if (KWndWindow::Init(pIniFile, pSection))
	{
		...
		m_MsgList.SetScrollbar(&m_Scroll);
