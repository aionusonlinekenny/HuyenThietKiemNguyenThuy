...
void Wnd_ProcessInput(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	//khi su dung chuot thi ngung auto
	if(uMsg == WM_LBUTTONDOWN || uMsg == WM_LBUTTONDOWN)
	{
		if (g_pCoreShell && g_pCoreShell->GetAutoFlag())
			g_pCoreShell->PAIOperation(GPI_F_MOUSE, true, NULL, NULL);
	}
	if(uMsg == WM_LBUTTONUP || uMsg == WM_LBUTTONUP)
	{
		if (g_pCoreShell)
			g_pCoreShell->PAIOperation(GPI_F_MOUSE, false, NULL, NULL);
	}
	//end code
	...
	{//====处理其他消息（其实都是键盘输入消息）====
		if (KPopupMenu::HandleInput(uMsg, uParam, nParam))
			return;
		//tu danh bang space by kinnox
		if (g_pCoreShell && g_pCoreShell->GetSpaceFlag())
		{
			if ((GetKeyState(VK_SPACE) & 0x8000))
				g_pCoreShell->PAIOperation(GPI_F_HOLDSPACE,true,NULL,NULL);
			else
				g_pCoreShell->PAIOperation(GPI_F_HOLDSPACE,false,NULL,NULL);	
		}
		else	
		{
			if ((GetKeyState(VK_SPACE) & 0x8000))
