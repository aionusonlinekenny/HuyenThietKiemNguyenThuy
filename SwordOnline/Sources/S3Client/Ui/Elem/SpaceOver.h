...
public:
    ...
    void    SetSpaceOverDesc(const char *pcszDescText, int nDescTextLen, UINT uColor);
...
private:
	...
	int     m_nDescLineNum;        //标题(名字)部分的行数
	int     m_nMaxLineLen;          //三部分一起算最长的行的长度

	unsigned int m_uDescBgColor;   //标题(名字)背景颜色
	unsigned int m_uDescColor;     //标题(名字)文字颜色
