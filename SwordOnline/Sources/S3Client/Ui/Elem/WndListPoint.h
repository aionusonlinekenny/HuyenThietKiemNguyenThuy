...
class KWndListPoint : public KWndWindow
{
public:
	virtual int	Init(KIniFile* pIniFile, const char* pSection);
	virtual int		WndProc(unsigned int uMsg, unsigned int uParam, int nParam);
	virtual void	PaintWindow();							
	int				AddValue(int nIndex, const int nValue1, const int nValue2, BOOL bHighLight = TRUE);
	int				GetString(int nIndex, char* pBuffer1, char* pBuffer2, unsigned int nSize);
	int				DeleteString(int nIndex);				
	void			ResetContent();							
	int				FindString(int nPrecedingStart, const char* pString);

	int				SetItemValue1(int nIndex, int nValue);		
	int				GetItemValue1(int nIndex);

	int				SetItemValue2(int nIndex, int nValue);		
	int				GetItemValue2(int nIndex);
	...
	KWndListPoint();
	virtual ~KWndListPoint();
protected:
	void			Clone(KWndListPoint* pCopy);
private:
