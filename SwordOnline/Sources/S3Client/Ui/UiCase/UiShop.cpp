...
#define	BUY_MESSAGES	"Ên gi÷ <color=green>Shift + chuét ph¶i<color> vµo vËt phÈm, lËp tøc mua ®­îc vËt phÈm."
...
{
	if(m_btBuyNumber > 0 && m_btBuyNumber <= MAX_BUY_NUMBER)
	{	
	...
	{	
		...
		else
		{
			...
			else
			{
				...
				sprintf(Msg.szMessage, BUY_MESSAGES);
...
void KUiShop::OnBuyItem(unsigned int uParam, BYTE btParam)
{	
	if(!uParam)
		return;
	
	KUiObjAtContRegion* pItem = (KUiObjAtContRegion*)uParam;

	m_ItemInfo = *pItem;

	if(btParam > 0 && btParam <= MAX_BUY_NUMBER)
	{
		m_btBuyNumber = btParam;
	}
	else
	{
		m_btBuyNumber = 1;
...
void KUiShop::SetPage(int nIndex)
{
	if (nIndex >= 0 && nIndex < m_nPageCount && m_pObjsList)
	{
		...
		//m_Text.Set2IntText(m_nCurrentPage + 1, m_nPageCount, '/');
	}
}
