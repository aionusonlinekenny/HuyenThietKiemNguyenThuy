...
void KUiRecovery::LoadScheme(KIniFile* pIni)
{
	...
	m_OtherEmeiBuffEdit.SetIntText(50);
	m_OtherSpamChatCK.Init(pIni, "OtherSpamChatCK");
	m_OtherSpamChatEdit.Init(pIni, "OtherSpamChatEdit");
	m_cMessenger = " (xh)<color=orange>§¹i Phong §ao<color> (dd)<color=red>AutoPlay<color> ";
...
void KUiRecovery::LoadRecoverySetting()
{
	...
	{			
		...
		pConfigFile->GetInteger(RTitle, "F18", 5, (int*)(&m_nValue));
		...
		pConfigFile->GetInteger(RTitle, "F27", 50, &m_nValue);
...
void KUiPick::OnPick()
{
	m_PickAllCK.CheckButton(m_PickAllCK.IsButtonChecked() > 0);
	g_pCoreShell->PAIOperation(GPI_P_BPICKUP,m_PickAllCK.IsButtonChecked(),NULL,NULL);	
}

void KUiPick::OnFillterItem()
{	
	m_PickFilterCK.CheckButton(m_PickFilterCK.IsButtonChecked() > 0);
	g_pCoreShell->PAIOperation(GPI_P_FILTEQUIP,m_PickFilterCK.IsButtonChecked(),NULL,NULL);	
}

void KUiPick::OnGiveItem() 
{	
	m_PickJewelryCK.CheckButton(m_PickJewelryCK.IsButtonChecked() > 0);	
	g_pCoreShell->PAIOperation(GPI_P_SAVERAP,m_PickJewelryCK.IsButtonChecked(),NULL,NULL);	
}

void KUiPick::OnCheckPrice()
{
	m_PickPriceItemsCK.CheckButton(m_PickPriceItemsCK.IsButtonChecked() > 0);	
	g_pCoreShell->PAIOperation(GPI_P_PRICEITEMCK,m_PickPriceItemsCK.IsButtonChecked(),NULL,NULL);	
}
void KUiPick::OnCheckLevel()
{
	m_PickLevelItemsCK.CheckButton(m_PickLevelItemsCK.IsButtonChecked() > 0);	
	g_pCoreShell->PAIOperation(GPI_S_LEVELITEMCK,m_PickLevelItemsCK.IsButtonChecked(),NULL,NULL);	
}
...
char szArray_FightBack[][64] =
{
	MSG_NON_SETTINGS,
	"Thæ ®Þa phï",
	"Chèng kÐo nh©n vËt vµo gãc lag",
};
//end code
...
void KUiFight::LoadFightSetting()
{
	...
	if (pConfigFile)
	{
		pConfigFile->GetInteger(FTitle, "F1", 1024, (int*)(&m_nValue));
		if (m_nValue <= 0)
			m_nValue =	1024;
		...
		pConfigFile->GetInteger(FTitle, "F7", 1, (int*)(&m_nValue));
		if (m_nValue > 0)
			m_nValue = true;
...
void KUiOther::LoadOtherSetting()
{
	...
	{			
		...
		pConfigFile->GetInteger(OTitle, "F2", 2, (int*)(&m_nValue));
		...
		pConfigFile->GetInteger(OTitle, "F3", 2, (int*)(&m_nValue));
