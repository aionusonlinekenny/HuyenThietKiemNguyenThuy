...
#define SCHEME_INI	"UiCreatPW.ini"

KUiCreatPW* KUiCreatPW::m_pSelf = NULL;


KUiCreatPW* KUiCreatPW::GetIfVisible()
{
...
KUiCreatPW* KUiCreatPW::OpenWindow()
{
	if (m_pSelf == NULL)
	{
		m_pSelf = new KUiCreatPW;
		if (m_pSelf)
			m_pSelf->Initialize();
	}
	if (m_pSelf)
...
// ¹¦ÄÜ	: ÔØÈë½çÃæ·½°¸
// -
void KUiCreatPW::LoadScheme(const char* pScheme)
{
	char		Buff[128];
	KIniFile	Ini;
...
// ¹¦ÄÜ	: ´°¿Úº¯Êý
// -
int KUiCreatPW::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	switch(uMsg)
	{
...
void KUiCreatPW::OnOk()
{
	if (OnCheckInput() == 1)
		return;
	g_pCoreShell->OperationRequest(GOI_PLAYER_CREATE_PWD,m_nPW1,0);
	KUiKeyBoard::CloseWindow();
	CloseWindow();
}

int KUiCreatPW::OnCheckInput()
{
	int nRet = 0;
