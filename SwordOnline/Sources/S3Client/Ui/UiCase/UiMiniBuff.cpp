...
#include "UiMiniBuff.h"
#include "UiAutoPlay.h"
#include "UiTongManager.h"
#include <crtdbg.h>

#define SCHEME_INI				"UiMiniBuff.ini"

UiMiniBuff* UiMiniBuff::m_pSelf = NULL;

extern iRepresentShell*	g_pRepresentShell;
extern iCoreShell *g_pCoreShell;


UiMiniBuff::UiMiniBuff()
{	
	m_BuffListCount = 0;
	m_nListCount = 0;
	m_pStateSkill = NULL;
...
UiMiniBuff* UiMiniBuff::OpenWindow()
{
	if (m_pSelf == NULL)
	{
		m_pSelf = new UiMiniBuff;
		if (m_pSelf)
			m_pSelf->Initialize();
	}
	if (m_pSelf)
...
/*********************************************************************
**********************************************************************/
UiMiniBuff* UiMiniBuff::GetIfVisible()
{
...
/*********************************************************************
**********************************************************************/
void UiMiniBuff::CloseWindow(bool bDestory)
{
	if (m_pSelf)
	{
		m_pSelf->Hide();
...
void UiMiniBuff::LoadScheme(const char* pScheme)
{
	if (m_pSelf)
	{
		char		Buff[128];
...
void UiMiniBuff::Add(int nSkillPos, KStateControl* pStateSkill)
{
...
void UiMiniBuff::Release()
{
	...
	// Gi?i phÛng b? nh? d„ c?p ph·t trong h‡m Breathe()
    if (m_pStateSkill)
    {
        free(m_pStateSkill);
        m_pStateSkill = NULL;
