...
#define	LEGEND_SETTING_SET		"\\Ui\\autoexec2.lua"
...
void KUiOptions::Initialize()
{
	...
	AddChild(&m_MusicText);
	AddChild(&m_MusicEx);
	AddChild(&m_SoundText);
	AddChild(&m_SoundEx);
	AddChild(&m_ShortcutText);
	//
	AddChild(&m_BGMValue);
...
void KUiOptions::LoadScheme(KIniFile* pIni)
{
	...
	m_MusicText.Init(pIni, "MusicText");
	m_MusicEx.Init(pIni, "MusicEx");
	m_SoundText.Init(pIni, "SoundText");
	m_SoundEx.Init(pIni, "SoundEx");
	m_ShortcutText.Init(pIni, "ShortcutText");
	char Buff[128];
	pIni->GetString("Settings", "EnableColor", "0,0,0", Buff, 128);
	m_uEnableTextColor = (GetColor(Buff) & 0x00ffffff);
	pIni->GetString("Settings", "DisableColor", "0,0,0", Buff, 128);
	m_uDisableTextColor = (GetColor(Buff) & 0x00ffffff);
	...
	{
		char	szKey[16];
		pIni->GetString("ToggleOptionsName", _itoa(i, szKey, 10), 
