...
KUiHelperMission* KUiHelperMission::m_pSelf = NULL;

#define		SCHEME_INI_HELP2	"UiHelperMission.ini"
#define		DETAIL_HELP_LIST	"HelpListMission.ini"

//--------------------------------------------------------------------------
//
//--------------------------------------------------------------------------
KUiHelperMission* KUiHelperMission::GetIfVisible()
{
...
//
//--------------------------------------------------------------------------
KUiHelperMission::KUiHelperMission()
{
	m_nIndexCount = 0;
}
...
//	
//--------------------------------------------------------------------------
KUiHelperMission* KUiHelperMission::OpenWindow(bool bShow)
{
	if (m_pSelf == NULL)
	{
		m_pSelf = new KUiHelperMission;
		if (m_pSelf)
			m_pSelf->Initialize();
	}
	if (m_pSelf)
...
//
//--------------------------------------------------------------------------
void KUiHelperMission::Initialize()
{
	AddChild(&m_BtnClose);
	//AddChild(&m_BtnPic);
...
//	
//--------------------------------------------------------------------------
void KUiHelperMission::CloseWindow(bool bDestory)
{
	if (m_pSelf)
	{	
		m_pSelf->Hide();
...
//
//--------------------------------------------------------------------------
void KUiHelperMission::LoadScheme(const char* pScheme)
{
	if (m_pSelf)
	{
		char		Buff[128];
		...
		if (Ini.Load(Buff))
		{
			...
			m_pSelf->m_Content.Init(&Ini, "Content");
...
//
//--------------------------------------------------------------------------
int KUiHelperMission::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	int nRet = 0;	
	switch(uMsg)
	{
...
//
//--------------------------------------------------------------------------
void KUiHelperMission::LoadIndexList(const char* pScheme)
{
	KIniFile	ListIni;
	int nCount;
	char szInifile[128] = "";
...
//
//--------------------------------------------------------------------------
void KUiHelperMission::UpdateData(int nIndex)
{
