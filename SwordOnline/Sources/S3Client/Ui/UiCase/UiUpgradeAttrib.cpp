...
void KUiUpgradeAttrib::LoadScheme(const char* pScheme)
{
	if (m_pSelf)
	{
		...
		m_pSelf->m_TextGuide.SetText("ß∆t trang bﬁ vµo Æ” xem thuÈc t›nh.");
	...
	case WND_N_BUTTON_CLICK:
		{
			...
			if (uParam == (unsigned int)&m_BtnUpgrade)
			{
				...
				if (ValidateUpgradeReady())
				{
					...
					UIMessageBox("Bπn c„ chæc lµ muËn n©ng thuÈc t›nh nµy?", this, "ßÂng ˝", "HÒy b·", ISP_DO_EVENT);
				}
				else
				{
...
BOOL KUiUpgradeAttrib::ValidateUpgradeReady()
{
	...
	if (pObj.uId == 0)
	{
		strcpy(szWarning, "Ch≠a c„ trang bﬁ nµo!");
...
void KUiUpgradeAttrib::DisplayAttributeList()
{
	...
	{
		m_TextGuide.SetText("Trang bﬁ nµy kh´ng c„ thuÈc t›nh!");
		return;
	}
...
void KUiUpgradeAttrib::OnSelectAttribute(int nSlot)
{
	...
	char szText[128];
	sprintf(szText, "Bπn ch‰n thuÈc t›nh: %d -> %d", m_Attributes[nSlot].nValue, m_Attributes[nSlot].nNewValue);
...
void KUiUpgradeAttrib::ClearAttributeList()
{
	...
	m_TextGuide.SetText("ß∆t trang bﬁ vµo Æ” xem thuÈc t›nh:");
}