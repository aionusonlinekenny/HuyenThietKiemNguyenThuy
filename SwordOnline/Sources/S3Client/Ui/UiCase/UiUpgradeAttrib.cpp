...
void KUiUpgradeAttrib::LoadScheme(const char* pScheme)
{
	if (m_pSelf)
	{
		...
		m_pSelf->m_TextGuide.SetText("§Æt trang bÞ xanh vµo sÏ xem thuéc tÝnh");
...
BOOL KUiUpgradeAttrib::ValidateUpgradeReady()
{
	...
	if (pObj.uId == 0)
	{
		strcpy(szWarning, "Ch­a ®Æt trang bÞ vµo!");
...
void KUiUpgradeAttrib::DisplayAttributeList()
{
	...
	{
		m_TextGuide.SetText("Trang bÞ nµy kh«ng cã thuéc tÝnh magic!");
		return;
	}
	...
	m_TextGuide.SetText("Chän thuéc tÝnh muèn n©ng cÊp:");
}
...
void KUiUpgradeAttrib::OnSelectAttribute(int nSlot)
{
	...
	char szText[128];
	sprintf(szText, "§· chän: %d -> %d", m_Attributes[nSlot].nValue, m_Attributes[nSlot].nNewValue);
...
void KUiUpgradeAttrib::ClearAttributeList()
{
	...
	m_TextGuide.SetText("§Æt trang bÞ xanh vµo ®Ó xem thuéc tÝnh:");
}