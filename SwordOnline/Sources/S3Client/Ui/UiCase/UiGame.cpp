...
void KUiGameSpace::OnMouseMoveCursor(int x, int y)
{
	...
	if (g_pCoreShell->FindSelectNPC(x, y, relation_all, true, &SelectPlayer, nKind))
	{
		...
		else if(g_pCoreShell->GetNPCShopState(SelectPlayer.nIndex))
			Wnd_SwitchCursor(MOUSE_CURSOR_SHOP);
		else
...
void ProcessPeople(KUiPlayerItem* pDest, int nAction)
{
	...
	case ACTION_TRADE:
		...
		break;
	case ACTION_CHAT:
		if(pDest->uId != 0 && g_pCoreShell->GetNPCShopState(pDest->nIndex))
		{
			g_pCoreShell->OperationRequest(GOI_VIEW_PLAYERSHOP, (unsigned int)pDest->uId, 0);
		}
...
void PopUpContextPeopleMenu(const KUiPlayerItem& SelectPlayer, int x, int y)
{
	...
	for (i = 0; i < nActionDataCount; i++)
	{
		if ((i == ACTION_JOINTEAM && SelectPlayer.nIndex != -1 && SelectPlayer.nData == PLAYER_MENU_STATE_TEAMOPEN) ||
			...
			(i == ACTION_MAKEFRIEND && !KUiChatCentre::IsMyFriend((char*)SelectPlayer.Name)) ||
			(i == ACTION_INVITETEAM && SelectPlayer.uId != 0 && TeamInfo.nCaptainPower > 0)	||
			...
			(i == ACTION_RQTONG && SelectPlayer.nIndex != -1 && nbIsRecruit) ||
			(i == ACTION_BLACKLIST && !IsInBlackName((char*)SelectPlayer.Name)) ||
			(i == ACTION_CHAT) ||
			(i == ACTION_OTT && SelectPlayer.nIndex != -1)
			)
		{
