...
//--------------------------------------------------------------------------
KUiTimeBox* KUiTimeBox::OpenWindow(const char* pszTitle, int nTime, char* pszAction,int nIndexNpc)
{
	if (m_pSelf == NULL)
	{
...
{
	g_pCoreShell->OperationRequest(GOI_TIME_BOX, (unsigned int)m_pSelf->szAction, (int)m_pSelf->nIndexNpc);
	CloseWindow(false);
}
