...
#include "UiCase/UiAutoPlay.h"//AI by kinnox;
...
#define GAME_STRING_FILE			"\\Ui\\Setting.ini"
...
char* l_WindowList[] =
{
	...
	"debug",
	"autoplay",
	"battlereport",//Report Battle By Kinnox;
	"tong",
	"openbox",
};
...
int FindWindow(const char* szname)
{
	...
	{
		if (_strcmpi(l_WindowList[i], szname) == 0)
			return i;
	}
	return -1;
}
...
int LuaOpenWindow(Lua_State * L)
{
	...
	if (strWindow)
	{
		...
		case 20:
			...
			break;
		case 21:
				KUiStoreBox::OpenWindow(-1);
			break;
		case 22: 
			if (KUiNewsMessageEx::GetIfVisible())
				KUiNewsMessageEx::CloseWindow();
			else
...
int FindStatus(const char* szname)
{
	...
	{
		if (_strcmpi(l_StatusList[i], szname) == 0)
			return i;
	}
	return -1;
}
			...
			if (g_pCoreShell)
			{
				if (!g_pCoreShell->GetGameData(GDI_PLAYER_OPEN_SHOP, 0, 0))
				{
					if (g_pCoreShell->GetAutoFlag())
						g_pCoreShell->PAIOperation(GPI_SWITCH_ACTIVE, FALSE, NULL, NULL);
					else
						g_pCoreShell->PAIOperation(GPI_SWITCH_ACTIVE, TRUE, NULL, NULL);
				}
			}
            break;
		}
...
int LuaGetAppStatus(Lua_State * L)
{
	...
	if (Lua_GetTopIndex(L) == 1)
	{
		...
		if (szName)
		{
			if (_strcmpi(szName, "FullWindow") == 0)
			{
				nRet = g_bScreen ? 1 : 0;
			}
			else if (_strcmpi(szName, "3D") == 0)
			{
				nRet = g_bRepresent3 ? 1 : 0;
			}
			else if (_strcmpi(szName, "2D") == 0)
			{
				nRet = !g_bRepresent3 ? 1 : 0;
			}
		}
...
int LuaDebug(Lua_State * L)
{
	...
	if (_strcmpi(strMode, "ShowMouseWnd") == 0)
	{
...
BOOL KShortcutKeyCentre::TranslateExcuteScript(const char * ScriptCommand)
{
	...
	{
		int nIndex = FindCommand(ScriptCommand);	
		if (nIndex >= 0)
		{
...
{
	//fix by kinnox;
	if (g_UiBase.GetStatus() == UIS_S_PLAYER_SHOP)
	{
		if ( ( (ScriptCommand == SCK_SHORTCUT_SYSTEM) || (ScriptCommand == SCK_SHORTCUT_OPTIONS) )&& ScriptCommand[0] != 0)
		{
	...
	else if ( g_UiBase.GetStatus() != UIS_S_IDLE || !ms_Enable) 
	{	
...
//
// --
int KShortcutKeyCentre::AddCommand(COMMAND_SETTING* pAdd)
{
	int nIndex = -1;
	if (pAdd == NULL)
		return nIndex;
...
//
// --
int	KShortcutKeyCentre::RemoveCommand(int nIndex)	
{
...
int	KShortcutKeyCentre::FindCommand(const char* szCommand)
{
	...
	for (int i = 0; i < ms_nCommands; i++)
	{
		if (ms_pCommands[i].szCommand[0] != 0 &&
			_strcmpi(ms_pCommands[i].szCommand, szCommand) == 0)
			return i;
	}
	return -1;
}
...
int	KShortcutKeyCentre::FindCommandByScript(const char* szScript)
{
	...
	for (int i = 0; i < ms_nCommands; i++)
	{
		if (ms_pCommands[i].szDo[0] != 0 &&
			_strcmpi(ms_pCommands[i].szDo, szScript) == 0)
			return i;
	}
	return -1;
}
