...
// Tr? v? 1 n?u decode OK, copy ra plain IPv4; n?u không ph?i ENC: thì copy nguyên van (gi? tuong thích)
...
// ==== [BEGIN] KLogin::GetServerList (dã c?p nh?t h? tr? ENC:<hex>) ====
        ...
        if (regionCount > 0)
        {
            ...
            if (nReadCount > 0)
            {
                ...
                if (pServers)
                {
                    ...
                    for (i = 0; i < nReadCount; i++)
                    {
                        // --- Ð?c Address: dùng buffer R?NG d? không c?t c?t ENC:<hex> ---
                        char szKey[32];
                        char szAddrRaw[256];   // raw t? file (ENC:<hex> dài)
                        ...
                        // Gi?i mã n?u có ENC:, ho?c gi? nguyên n?u là IPv4 thu?ng
