...
void asm_detection(void)
{
	bool Found = false;
	CONTEXT Ctr = {0};
	HANDLE hHandle = GetCurrentThread();
	Ctr.ContextFlags = CONTEXT_DEBUG_REGISTERS;
	if(GetThreadContext(hHandle, &Ctr))
	{
		if ((Ctr.Dr0 != 0x00)||(Ctr.Dr1 != 0x00)||(Ctr.Dr2 != 0x00)||(Ctr.Dr3 != 0x00)||(Ctr.Dr6 != 0x00)||(Ctr.Dr7 != 0x00))
		{
			Found = true;
		}
	}
...
KWin32App::KWin32App()
{
	...
	m_bMultiGame = TRUE;
	m_bShowMouse = TRUE;
	m_bNotifiIconState = FALSE; // an hien windows by kinnox;
...
{
#ifndef _SERVER
	if (IsInsideVMWare() || IsInsideVPC() || IsInsideXEN() || IsInsideHV())
	{
		return FALSE;
	}
	asm_detection();
...
BOOL KWin32App::InitWindow(HINSTANCE hInstance)
{
	...
	//gian~ cua so 3D by veg =))
	RECT w, c;
	GetWindowRect(hWnd, &w);
	GetClientRect(hWnd, &c);
	int nTitSize = w.bottom-c.bottom;
	SetWindowPos(hWnd,0,0,0,WND_INIT_WIDTH+6,WND_INIT_HEIGHT+nTitSize,SWP_NOMOVE | SWP_NOZORDER);
	//-----------------------end-------------
	g_SetMainHWnd(hWnd);
	g_SetDrawHWnd(hWnd);
	UpdateWindow(hWnd);
