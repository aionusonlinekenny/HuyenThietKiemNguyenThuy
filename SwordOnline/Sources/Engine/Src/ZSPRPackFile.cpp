...
bool ZPackFile::_readData(int node_index, char *node) {
	...
	int r = 1;
	if((compress_type == TYPE_UCL) | (compress_type == TYPE_UCL2) | (compress_type == TYPE_BZIP2)) {									//整体压缩
		if((compress_type == TYPE_UCL) | (compress_type == TYPE_UCL2)) {
				...
				else
				{
					...
					if (pcBuffer)
					{
						...
						{							
							...
							{
								if((uCompressType == TYPE_UCL) || (uCompressType == TYPE_UCL2))
								{
