...
// --
bool KPakList::FindElemFile4Unpack(unsigned long uId, XPackElemFileRef& ElemRef, int nIndex)
{
	bool bFounded = false;
	if(nIndex < 0)
	{
	...
	{
		if (m_PakFilePtrList[nIndex]->FindElemFile(uId, ElemRef))
		{
			bFounded = true;
		}
	}
...
// --
bool KPakList::FindElemFile4Unpack(const char* pszFileName, XPackElemFileRef& ElemRef, int nIndex)
{
	bool bFounded = false;
	if (pszFileName && pszFileName[0])
	{
		...
		g_GetPackPath(szPackName + 1, (char*)pszFileName);
		unsigned long uId = FileNameToId(szPackName);
		bFounded = FindElemFile4Unpack(uId, ElemRef, nIndex);
	}
	return bFounded;
}
#endif
