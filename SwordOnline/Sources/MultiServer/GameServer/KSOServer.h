...
struct GameStatus 
{
	GameStatus() 
	{
		...
		nSendPingTime = 0;
		nReplyPingTime = 1;
		nPlayerIndexLixian = 0; //Lixian by kinnox;
		nPingHistCount = 0;
        for (int k = 0; k < 4; ++k)
            nPingHistory[k] = 0;
	};
	...
	int					nPlayerIndexLixian;//Lixian by kinnox;	
	int nPingHistory[4];  
    int nPingHistCount;
} ;
...
private:
	...
	void				SavePlayerData();	
	BOOL				SavePlayerData(int nIndex, bool bUnLock, int nSaveType = 0);//Lixian by kinnox;
	void				ExitAllPlayer();
