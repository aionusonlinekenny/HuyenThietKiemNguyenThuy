...
void CClientNode::_SaveRoleInfo( const void *pData, size_t dataLength )
{
	...
	if (pPD->nSaveType == 0) //Lixian by kinnox;
	{
...
void CClientNode::_DelRole( const void *pData, size_t dataLength )
{
	...
	nDataLen = nDataLen - sizeof(TProcessData) - 15;		
	...
	{	
		// 
		memcpy( szAccountName, &pRoleInfo->pDataBuffer[0], nDataLen );
		szAccountName[nDataLen] = '\0';
				
		memcpy( szRoleName, &pRoleInfo->pDataBuffer[nDataLen], 16);
		szRoleName[16] = '\0';
	
	
		S3DBI_RoleBaseInfo DBI[4];
		char cCount = (char)GetRoleListOfAccount( szAccountName, &DBI[0], 3 );
		char cStepCheck = 0;
		if(cCount > 0)
		{
			...
			{	
				if(!strcmp(DBI[i].szName, szRoleName))
				{	
					if(DBI[i].dwTongId)
					{	
						result = -3;
...
void CClientNode::_GetRoleInfo( const void *pData, size_t dataLength )
{
	...
	if ( nDataLen > 0 )
	{
		...
		if(pRoleInfo->pDataBuffer[nDataLen] == 1)
			GetRoleInfo( &pRoleData->pDataBuffer[1], szRoleName, nRoleInfoLen );
		else if( !IsRoleLock(szRoleName) )
			GetRoleInfo( &pRoleData->pDataBuffer[1], szRoleName, nRoleInfoLen );
