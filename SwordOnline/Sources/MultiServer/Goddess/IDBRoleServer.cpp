...
void ReleaseDBInterface()       //释放数据库引擎
{
    if ( db_table )
    {
        db_table->commit();
/*
        // NOTE: sua dieu kien kiem tra handle
        if (hDeadLockThread) {
            TerminateThread(hDeadLockThread, 0); // NOTE: han che dung TerminateThread, giu nguyen phong cach code cu
            CloseHandle(hDeadLockThread);        // them: dong handle
            hDeadLockThread = NULL;              // them: set NULL
        }
*/
        // NOTE: sua dieu kien kiem tra handle
        if (hRemoveLogThread) {
            TerminateThread(hRemoveLogThread, 0); // NOTE: han che dung TerminateThread, giu nguyen phong cach code cu
            CloseHandle(hRemoveLogThread);        // them: dong handle
            hRemoveLogThread = NULL;              // them: set NULL
        }
        ...
        db_table->close();
        delete db_table;
        db_table = NULL; //them code 16/09/2023 by kinnox;
    }
}
...
//      char *buffer = db_table->search( szAccountName, strlen( szAccountName ) + 1, size, 0 );
        ...
        if ( nCount >= (int)nMaxRoleCount_InAccount )
        {
            return 0;
        }
    }
...
//  char *buffer = db_table->search( szAccountName, strlen( szAccountName ) + 1, size, 0 );
    ...
    while ( count < nMaxCount && cursor )
    {
        ...
        base_ptr->dwTongId = pRoleData->BaseInfo.dwTongID;      
...
{//开始运行备份线程
    // NOTE: co the goi nhieu lan; neu DBBackup != NULL thi nen dung truoc
    // (giu nguyen hanh vi hien tai neu ban khong muon dung truoc)
...
{//结束运行备份线程
    ...
    // NOTE: tranh busy-wait an CPU, them Sleep
    while(IsBackupWorking()){
        Sleep(10); // them: nghi 10ms moi vong
    }
...
{//手工备份
    if(!DBBackup)return false;
    // NOTE: ManualBackup() se tu chan neu dang co backup khac (khoa re-entrancy)
