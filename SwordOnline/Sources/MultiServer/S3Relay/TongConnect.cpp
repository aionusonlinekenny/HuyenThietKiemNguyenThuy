...
int	g_nTongPSSize[defTONG_PROTOCOL_SERVER_NUM] = 
{
	...
	sizeof(STONG_CHANGE_MEMBER_TITLE_COMMAND),	// enumC2S_TONG_CHANGE_MEMBER_TITLE
	sizeof(STONG_CHANGE_ONE_TITLE_COMMAND),		// enumC2S_TONG_CHANGE_ONE_TITLE
	sizeof(STONG_SAVE_MONEY_COMMAND),			// enumC2S_TONG_SAVE_MONEY
	sizeof(STONG_GET_MONEY_COMMAND),			// enumC2S_TONG_GET_MONEY
...
void CTongConnect::Proc0_Tong(const void* pData, size_t size)
{
	...
	case enumC2S_TONG_CREATE:
		{	
			char	szPlayerName[defPLAYER_NAME_MAX_LENGTH + 1], szTongName[defTONG_NAME_MAX_LENGTH + 1];
			STONG_CREATE_COMMAND	*pTongCreate = (STONG_CREATE_COMMAND*)pData;
			int		nCamp = pTongCreate->m_btCamp;
			BOOL	bGender = pTongCreate->m_bGender;
			...
			int nErrorID = g_cTongSet.Create(nCamp, szPlayerName, szTongName, bGender);
			if (nErrorID == 0)
			{
	...
	case enumC2S_TONG_ADD_MEMBER:
		{	
			...
			int nRet = g_cTongSet.AddMember(szPlayerName, szTongName, bGender);
			if (nRet >= 0)
			{
				...
				g_cTongSet.GetMemberTitle(nRet, sSync.m_szTitleName, bGender);
	...
	case enumC2S_TONG_CHANGE_CAMP:
		{	
			STONG_CHANGE_CAMP_COMMAND* pChange = (STONG_CHANGE_CAMP_COMMAND*)pData;
			STONG_CHANGE_CAMP_SYNC	sChange;
			sChange.ProtocolID		= 0;
			g_cTongSet.ChangeCamp(pChange, &sChange);
			if (sChange.ProtocolID != 0)
				this->SendPackage((const void *)&sChange, sizeof(sChange));
		}
		break;
	case enumC2S_TONG_CHANGE_MEMBER_TITLE:
		{	
			STONG_CHANGE_MEMBER_TITLE_COMMAND* pChange = (STONG_CHANGE_MEMBER_TITLE_COMMAND*)pData;
			STONG_CHANGE_MEMBER_TITLE_SYNC	sChange;
			sChange.ProtocolID		= 0;
			g_cTongSet.ChangeMemberTitle(pChange, &sChange);
			if (sChange.ProtocolID != 0)
				this->SendPackage((const void *)&sChange, sizeof(sChange));
		}
		break;
	case enumC2S_TONG_CHANGE_ONE_TITLE:
		{	
			STONG_CHANGE_ONE_TITLE_COMMAND* pChange = (STONG_CHANGE_ONE_TITLE_COMMAND*)pData;
			STONG_CHANGE_ONE_TITLE_SYNC	sChange;
			sChange.ProtocolID = 0;
			g_cTongSet.ChangeOneTitle(pChange, &sChange);
			if (sChange.ProtocolID != 0)
				this->SendPackage((const void *)&sChange, sizeof(sChange));

		}
		break;
	case enumC2S_TONG_SAVE_MONEY:
		{
			STONG_SAVE_MONEY_COMMAND* pChange = (STONG_SAVE_MONEY_COMMAND*)pData;
			STONG_SAVE_MONEY_SYNC sChange;

			sChange.ProtocolID = 0;
			if(!g_cTongSet.SaveMoney(pChange, &sChange))
			{
				STONG_SAVE_MONEY_FAIL_SYNC	sFail;
				sFail.ProtocolFamily	= pf_tong;
				sFail.ProtocolID		= enumS2C_TONG_SAVE_MONEY_FAIL;
				sFail.m_dwTongNameID	= pChange->m_dwTongNameID;
				sFail.m_dwParam			= pChange->m_dwParam;
				sFail.m_nMoney			= pChange->m_nMoney;
				this->SendPackage((const void *)&sFail, sizeof(sFail));							
				break;
			}
		...
		break;
	case enumC2S_TONG_GET_MONEY:
		{
			STONG_GET_MONEY_COMMAND* pChange = (STONG_GET_MONEY_COMMAND*)pData;
			STONG_GET_MONEY_SYNC sChange;
			sChange.ProtocolID = 0;
			g_cTongSet.GetMoney(pChange, &sChange);
			if(sChange.ProtocolID != 0)
				this->SendPackage((const void *)&sChange, sizeof(sChange));
		}
		break;
