...
static const size_t MAX_SENTLEN = 512;		//
...
CChannelMgr::CChannelMgr()
	...
	m_minTeamID(0), m_maxTeamID(-1), m_minFacID(0), m_maxFacID(-1), m_minTongID(0), m_maxTongID(-1), m_minBattleID(0), m_maxBattleID(-1),
	m_defCost(0),
	m_costTeam(0), m_costFac(0), m_costTong(0), m_costScrn(0), m_costBc(0), m_costBattle(0)
{
...
BOOL CChannelMgr::Initialize()
{
	...
	m_chChannBATTLE = gGetPrivateProfileCharEx(sec_battle, key_escSpec, file_channcfg, defCHANN_BATTLE);
	...
	m_minBattleID	= gGetPrivateProfileIntEx(sec_battle, key_minID, file_channcfg, 0);
	m_maxBattleID	= gGetPrivateProfileIntEx(sec_battle, key_maxID, file_channcfg, -1);
	...
	m_costBattle  = (BYTE)gGetPrivateProfileIntEx(sec_battle, key_cost, file_channcfg, (INT)(UINT)m_defCost);
	...
	memset(&m_ArrPaketAnti, 0, sizeof(m_ArrPaketAnti));
...
std::_tstring CChannelMgr::MakeChannelName(const std::_tstring& chann, DWORD ip)
{
		...
		|| chann[1] == m_chChannBATTLE)
	{
		...
		if (chann[1] == m_chChannBATTLE)
			return IsValidBattleID(id) ? chann : NULL_STR;
...
BYTE CChannelMgr::GainPreCost(const std::_tstring& channel)
{
	if (channel.size() >= 2 && channel[0] == m_chChannEsc)
	{
		...
		else if (channel[1] == m_chChannBATTLE)
			return m_costBattle;
...
BOOL CChannelMgr::IsValidBattleID(DWORD idBattle)
{
	if (m_minBattleID > m_maxBattleID)
		return TRUE;

	return idBattle >= m_minBattleID && idBattle <= m_maxBattleID;
}

BOOL CChannelMgr::AddAntiPaket(DWORD srcIp, DWORD timer)
{
	int i = 0;
	for(i = 0; i < MAX_PLAYER_ANTI_PAKET; i++){

		if (m_ArrPaketAnti[i][0] == 0) break;

		if (m_ArrPaketAnti[i][0] == srcIp){
			if (m_ArrPaketAnti[i][1] == timer) return FALSE;
			else break;

		}
	}

	if (i == MAX_PLAYER_ANTI_PAKET - 1){
		memset(&m_ArrPaketAnti, 0,sizeof(m_ArrPaketAnti));
		i =0;
	}

	m_ArrPaketAnti[i][0] = srcIp;
	m_ArrPaketAnti[i][1] = timer;
...
BOOL CChannelMgr::SomeoneChat(DWORD srcIP, const tagPlusSrcInfo& SrcInfo, const CHAT_SOMEONECHAT_CMD* pSomeoneChatCmd)
{
	...
	if (!AddAntiPaket(srcIP, pSomeoneChatCmd->dwTimePacker))
		return FALSE;
...
BOOL CChannelMgr::ChannelChat(DWORD srcIP, const tagPlusSrcInfo& SrcInfo, const CHAT_CHANNELCHAT_CMD* pChannelChatCmd)
{
	...
	if (!AddAntiPaket(srcIP, pChannelChatCmd->dwTimePacker))
		return FALSE;
	...
	else
	{
		...
		{//relegate to gamesvr
			...
			else if (rChannInfo.channname[1] == m_chChannBATTLE)
			{
				if (theID == -1)
					return FALSE;

				pCr->TargetCls = tgtcls_battle;
				pCr->TargetID = theID;
