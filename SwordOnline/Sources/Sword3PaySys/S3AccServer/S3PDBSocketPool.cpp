...
void KGatewayDataProcess::AutoTime()
{
	...
	if (m_nLeftStatusTime > 0 && (nNow - m_nStatusTime > (unsigned long)m_nLeftStatusTime) && IsWorkAgain())
	{
	...
	if (m_nLeftPingTime > 0 && (nNow - m_LastPingTime) >= (unsigned long)m_nLeftPingTime && IsWork())
	{
		m_pServer->ShutdownClient(m_nConnectID);
	}
...
BOOL KGatewayDataProcess::ProAccountLogin(S3PDBConVBC* pConn, const IBYTE* lpData, const DWORD dwDataSize)
{
	...
	if (NULL != lpData && dwDataSize == sizeof(KAccountUserLoginInfo))
	{
			...
			)
		{
			...
			if (m_UserNames.find(*((UserName*)pAccInfo->Account)) == m_UserNames.end())
			{
				if (pAccInfo->Type == AccountUserLoginInfo)
					aReturn.nReturn = S3PAccount::Login(pConn, pAccInfo->Account, pAccInfo->Password, pAccInfo->License, m_nGameID, aReturn.nLeftTime);
				else
...
BOOL KGatewayDataProcess::ProGameLogin(S3PDBConVBC* pConn, const IBYTE* lpData, const DWORD dwDataSize)
{
	...
	if (NULL != lpData && dwDataSize == sizeof(KAccountUser))
	{
		...
		if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION &&
			(pAccInfo->Type == AccountUserLoginInfo || pAccInfo->Type == AccountUser)
			)
		{	
			//
			KAccountUserReturnExt aReturn;
			...
			memcpy(aReturn.Account, pAccInfo->Account, LOGIN_USER_ACCOUNT_MAX_LEN);
			if (pAccInfo->Type == AccountUserLoginInfo)
				aReturn.nReturn = S3PAccount::QueryExtPoint(pConn, m_nGameID, pAccInfo->Account, aReturn.nLeftTime);
			else
			...
			if (pAccInfo->Type == AccountUserLoginInfo)
				gTrace("Gateway %s(%s): User %s Query ExtPoint%s (ExtPoint:%d)", m_ServerName, inet_ntoa(ar), pAccInfo->Account, szDetail, 0);
			else
...
DWORD KGatewayDataProcess::Main(LPVOID lpParam)
{
	...
	{
		S3PAccount::ElapseAll(m_pConn, m_nGameID);
	}
...
bool KGatewayDataProcess::IsVerifyTimeout()
{
	...
	{
		if ((GetTickCount() - m_nStatusTime) <= (DWORD)m_nLeftStatusTime)
		{	//正确状态的有限时间内,才叫不超时
			bRet = false;
		}
	}
