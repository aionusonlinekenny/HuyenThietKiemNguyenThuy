...
extern int g_NpcTimerCallBackFun(void * pOwner, char * szScriptFileName);
...
class KMission
{
	...
	char m_MissionString[MAX_MISSION_VALUE_COUNT][MAX_PATH];
...
public:
	...
	KMission()
	{
		...
		memset(m_MissionString, 0, sizeof(m_MissionString));
		m_ulMissionId = 0;
	};
	BOOL	Activate();
	...
	void SetMissionString(unsigned long ulValueId, const char* lpszString)
	{
		if (ulValueId >= MAX_MISSION_VALUE_COUNT)
			return ;
		memcpy(&m_MissionString[ulValueId], lpszString, sizeof(m_MissionString[ulValueId]));
	};
	...
	void GetMissionString(unsigned long ulValueId, char* lpszResult)
	{
		if (ulValueId >= MAX_MISSION_VALUE_COUNT)
			return;

		memcpy(&lpszResult, m_MissionString[ulValueId], sizeof(m_MissionString[ulValueId]));
	};
