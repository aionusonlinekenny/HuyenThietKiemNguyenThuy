...
//关于不隶属于场景地图的对象与场景关系的操作
	...
	void 	GetSceneCalgFlagPos(int& nX, int& nY);
	...
	void CalcFlagPos(int nX, int nY, bool bCalc, bool bLine);//PaintFindPos by kinnox;
...
private:
	...
	enum SP_WORKING_PARAM { 
		...
		//##Documentation
		//## 最多加载的区域的数目
		SPWP_MAX_NUM_REGIONS = 49, //m_RangePosTable 7x7 = 49;
		//##Documentation
		//## The maximum number of regions that can enter the processing range
...
private:
	...
	//char m_szSceneName[32]; sua cho nay by kinnox;
...
private:
    ...
    //According to the direction the player is moving, the Region that needs to be loaded
    KPrevLoadFileNameAndFrame   m_PrevLoadFileNameAndFrames[MAX_PREV_LOAD_FILE_COUNT];
	
	KRUImage*		m_pSceneBgImg;
	BYTE			m_btSceneBgImgCount;
