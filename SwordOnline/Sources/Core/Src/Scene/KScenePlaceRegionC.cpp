...
KScenePlaceRegionC::KScenePlaceRegionC()
{
	...
	m_MoveTopCornerScenePos.x = m_MoveTopCornerScenePos.y = 0;
...
bool KScenePlaceRegionC::Load(const char* pszBaseFolderName)
{
	...
	m_MoveTopCornerScenePos.x = m_RegionIndex.x * RWPP_AREGION_WIDTH;
	m_MoveTopCornerScenePos.y = m_RegionIndex.y * RWPP_AREGION_HEIGHT;
	char	File[256];
...
void KScenePlaceRegionC::PaintOverCloud()
{
	if (g_pRepresent == NULL || SubWorld[0].m_SubWorldID != 2 && SubWorld[0].m_SubWorldID != 53)
		return;
	...
	const int SLOW_DOWN_RATE = 50;//121;
	KSPRCrunode* pGrunode = m_GroundLayerData.pGrunodes;
	...
	for (nIndex = 0; nIndex < m_GroundLayerData.uNumGrunode; nIndex++)
	{
			pGi->nType = ISI_T_SPR;
			pGi->Color.Color_b.a = 255;
			pGi->bRenderStyle = IMAGE_RENDER_STYLE_ALPHA_NOT_BE_LIT;
			if (counter % SLOW_DOWN_RATE == 0)
			{
				pGi->oPosition.nX = pGrunode->Param.h * CellWidth + m_MoveTopCornerScenePos.x--;
				//g_DebugLog("toa do  %d",pGi->oPosition.nX);
				if (pGi->oPosition.nX <  ((pGrunode->Param.h * CellWidth + m_LeftTopCornerScenePos.x) - 1200))
				{
					m_MoveTopCornerScenePos = m_LeftTopCornerScenePos;
					pGi->oPosition.nX = pGrunode->Param.h * CellWidth + m_MoveTopCornerScenePos.x;
				}
			}	
			else
			pGi->oPosition.nX = pGrunode->Param.h * CellWidth + m_MoveTopCornerScenePos.x;
			pGi->oPosition.nY = (pGrunode->Param.v * CellHeight) * 2 + m_MoveTopCornerScenePos.y;
			counter++;
			
			if (nIndex == 7 || nIndex == 14 || nIndex == 21 || nIndex == 28 || nIndex == 35 || nIndex == 42 || nIndex == 49 || nIndex == 56)
				memcpy(pGi->szImage, "\\游戏资源\\background\\cloud5.spr", pGrunode->Param.nFileNameLen);
			else if (nIndex == 3 || nIndex == 9 || nIndex == 15 || nIndex == 27 || nIndex == 33 || nIndex == 39 || nIndex == 51 || nIndex == 57)
				memcpy(pGi->szImage, "\\游戏资源\\background\\cloud1.spr", pGrunode->Param.nFileNameLen);
			
			else if (nIndex == 5 || nIndex == 13 || nIndex == 21 || nIndex == 29 || nIndex == 37 || nIndex == 45 || nIndex == 53 || nIndex == 61)
				memcpy(pGi->szImage, "\\游戏资源\\background\\cloud8.spr", pGrunode->Param.nFileNameLen);
			else if (nIndex == 4 || nIndex == 12 || nIndex == 20 || nIndex == 28 || nIndex == 36 || nIndex == 44 || nIndex == 52 || nIndex == 60)
				memcpy(pGi->szImage, "\\游戏资源\\background\\cloud2.spr", pGrunode->Param.nFileNameLen);
			else if (nIndex == 6 || nIndex == 11 || nIndex == 22 || nIndex == 27 || nIndex == 38 || nIndex == 47 || nIndex == 58 || nIndex == 63)
				memcpy(pGi->szImage, "\\游戏资源\\background\\cloud6.spr", pGrunode->Param.nFileNameLen);
			else if (nIndex == 6 || nIndex == 11 || nIndex == 22 || nIndex == 27 || nIndex == 38 || nIndex == 47 || nIndex == 58 || nIndex == 63)
				memcpy(pGi->szImage, "\\游戏资源\\background\\cloud9.spr", pGrunode->Param.nFileNameLen);
			else if (nIndex == 8 || nIndex == 16 || nIndex == 24 || nIndex == 32 || nIndex == 40 || nIndex == 48 || nIndex == 56 || nIndex == 64)
				memcpy(pGi->szImage, "\\游戏资源\\background\\cloud7.spr", pGrunode->Param.nFileNameLen);
			else
				memcpy(pGi->szImage, "\\游戏资源\\background\\cloud4.spr", pGrunode->Param.nFileNameLen);
			pGi->nFrame = pGrunode->Param.nFrame;
			pGi->uImage = 0;
			pGi->nISPosition = IMAGE_IS_POSITION_INIT;
			pGrunode = (KSPRCrunode*)(((char*)pGrunode) +
			...
			{
				g_pRepresent->DrawPrimitives(1, &ImgList[0], RU_T_IMAGE, false);
				nNum = 0;
				pGi = &ImgList[0];
			}
	}
	...
	if (nNum)
		g_pRepresent->DrawPrimitives(1, &pGi[0], RU_T_IMAGE, false);
}
// --
void KScenePlaceRegionC::PaintOverCloud_up()
{
	if (g_pRepresent == NULL || SubWorld[0].m_SubWorldID != 2 && SubWorld[0].m_SubWorldID != 53)
		return;
	...
	for (nIndex = 0; nIndex < m_GroundLayerData.uNumGrunode; nIndex++)
	{
		...
		if (counter % SLOW_DOWN_RATE == 0)
		{
			pGi->oPosition.nY = pGrunode->Param.v * CellHeight + m_MoveTopCornerScenePos.y--;
			//g_DebugLog("toa do  %d",pGi->oPosition.nX);
			if (pGi->oPosition.nY <  ((pGrunode->Param.v * CellHeight + m_MoveTopCornerScenePos.y) + 600))
			{
				m_MoveTopCornerScenePos = m_MoveTopCornerScenePos;
				pGi->oPosition.nY = pGrunode->Param.v * CellHeight + m_MoveTopCornerScenePos.y;
			}
		}	
		else
...
// --
void KScenePlaceRegionC::PaintGround(bool bBgImg /*= false*/)
{
	if(m_pPrerenderGroundImg && !bBgImg)
	{	
