...
//////////////////////////////////////////////////////////////////////
ZEG_MSSQLServer_Connection::ZEG_MSSQLServer_Connection()
...
void ZEG_MSSQLServer_Connection::CloseConnect()
{
	if(NULL != m_pCon)
	{
		m_pCon->Close();
...
bool ZEG_MSSQLServer_Connection::OpenConnect(_LPDATABASEINFO lpDBIdentifier)
{
	...
	if (NULL == m_pCon)
	{
			...
			&& (NULL != pCon))
		{
			...
			{
				printf("[SQLConnection::OpenConnect]COM error: %s\n", e.ErrorMessage());
			}
		}
	}
	return bRet;
...
bool ZEG_MSSQLServer_Connection::Do(const char* lpszSql)
{
	if (m_pCmd)
...
bool ZEG_MSSQLServer_Connection::DoCmd(const char* lpszSql)
{
	BOOL bRet = false;
    ...
    {
		printf("[ZEG_MSSQLServer_Connection::Do COM error: %s\n", e.ErrorMessage());
    }

	return bRet;
}

bool ZEG_MSSQLServer_Connection::DoResult(const char* lpszSql)
{
	BOOL bRet = false;
	if (m_pCon == NULL)
		return bRet;
...
bool ZEG_MSSQLServer_Connection::QuerySql(const char* lpszSql, ZEG_MSSQLServer_Result** ppResult)
{
	*ppResult = NULL;
	BOOL bRet = FALSE;
...
bool ZEG_MSSQLServer_Connection::QueryResult(const char* lpszSql, ZEG_MSSQLServer_Result* pRes)
{
	BOOL bRet = false;
	if ((NULL != m_pCon) &&
		(NULL != lpszSql) &&
		(NULL != pRes))
	{
		...
		catch ( _com_error &e )
		{
			...
			printf("SQL Error: %s\n", lpszSql);
#endif
			printf("[ZEG_MSSQLServer_Connection::DoSql COM error: %s\n", e.ErrorMessage());
...
bool ZEG_MSSQLServer_Connection::QueryCmd(const char* lpszSql, ZEG_MSSQLServer_Result* pRes)
{
	BOOL bRet = false;
	if ((NULL != m_pCon) &&
		(NULL != lpszSql) &&
...
bool ZEG_MSSQLServer_Connection::GetFreeResult(ZEG_MSSQLServer_Result** ppResult)
{
	assert(ppResult);
