...
void	KPlayerTeam::InviteAdd(DWORD dwNpcID)
{
	...
	sAdd.dwID = 0;			//Tam bo~ by kinnox;	
	sAdd.dwTimePacker = 0; //Tam bo~ by kinnox;	
...
void	KPlayerTeam::ReceiveInvite(TEAM_INVITE_ADD_SYNC *pInvite)
{
	...
	{
		if (Player[CLIENT_PLAYER_INDEX].m_cAI.m_bAutoAccept && 
			Player[CLIENT_PLAYER_INDEX].m_cAI.m_bAutoAcceptAll)
		{
			ReplyInvite(nIdx, 1);
			return;
		}
		else if (Player[CLIENT_PLAYER_INDEX].m_cAI.m_bAutoAccept && 
				 Player[CLIENT_PLAYER_INDEX].m_cAI.m_bAutoAcceptWithName &&
...
void	KPlayerTeam::GetInviteReply(int nSelfIdx, int nTargetIdx, int nResult)
{
	...
	sAddMem.m_TP = SubWorld[Npc[Player[nTargetIdx].m_nIndex].m_SubWorldIndex].m_SubWorldID; // L?y SubWorldID t? SubWorld
	sAddMem.m_SM = Player[nTargetIdx].m_cFaction.m_nCurFaction;
	sAddMem.m_HP = Npc[Player[nTargetIdx].m_nIndex].m_CurrentLife;
	sAddMem.m_MHP = Npc[Player[nTargetIdx].m_nIndex].m_CurrentLifeMax;
	strcpy(sAddMem.m_szName, Npc[Player[nTargetIdx].m_nIndex].Name);
	//Kenny Name Fix
	/*strncpy(sAddMem.m_szName, Npc[Player[nTargetIdx].m_nIndex].Name, sizeof(sAddMem.m_szName) - 1);
	sAddMem.m_szName[sizeof(sAddMem.m_szName) - 1] = '\0';*/
...
// - 给新队员发消息 // --
	...
	sSelfInfo.m_SM[0] = Player[nSelfIdx].m_cFaction.m_nCurFaction;
	sSelfInfo.m_HP[0] = Npc[nNpcIndex].m_CurrentLife; // Lay HP th鄋h vi阯
	sSelfInfo.m_MHP[0] = Npc[nNpcIndex].m_CurrentLifeMax; // Lay HP th鄋h vi阯
	strcpy(sSelfInfo.m_szNpcName[0], Npc[nNpcIndex].Name);
	/*strncpy(sSelfInfo.m_szNpcName[0], Npc[nNpcIndex].Name, sizeof(sSelfInfo.m_szNpcName[0]) - 1);
	sSelfInfo.m_szNpcName[0][sizeof(sSelfInfo.m_szNpcName[0]) - 1] = '\0';*/
	...
	for (i = 0; i < MAX_TEAM_MEMBER; i++)
	{
		if (g_Team[m_nID].m_nMember[i] >= 0)
		{
			...
			sSelfInfo.m_SM[i + 1] = Player[nSelfIdx].m_cFaction.m_nCurFaction;
			sSelfInfo.m_HP[i + 1] = Npc[nNpcIndex].m_CurrentLife; // Lay HP th鄋h vi阯
			sSelfInfo.m_MHP[i + 1] = Npc[nNpcIndex].m_CurrentLifeMax; // Lay HP th鄋h vi阯	
			strcpy(sSelfInfo.m_szNpcName[i + 1], Npc[nNpcIndex].Name);
			/*strncpy(sSelfInfo.m_szNpcName[i + 1], Npc[nNpcIndex].Name, sizeof(sSelfInfo.m_szNpcName[i + 1]) - 1);
			sSelfInfo.m_szNpcName[i + 1][sizeof(sSelfInfo.m_szNpcName[i + 1]) - 1] = '\0';*/
...
// --
void	KTeam::CreateTeam(int nCaptainNpcID, char *lpszCaptainName, int nCaptainLevel, DWORD nTeamServerID, int nCaptainHP, int nCaptainMHP, int nCaptainSM, int nCaptainTP)
{
	Release();
	m_nCaptain = nCaptainNpcID;
	m_nMemTP[0] = nCaptainTP;
	m_nMemSM[0] = nCaptainSM;
	m_nMemLevel[0] = nCaptainLevel;
	m_nMemHP[0] = nCaptainHP;////Kenny
	m_nMemMHP[0] = nCaptainMHP;
...
//	功能：添加一个队伍成员
// --
BOOL	KTeam::AddMember(DWORD dwNpcID, int nLevel, char *lpszNpcName, int nHP, int nMHP, int nSM, int nTP)
{
	for (int i = 0; i < MAX_TEAM_MEMBER; i++)
	{
	...
	m_nMemSM[nFreeNo + 1] = nSM;
	m_nMemTP[nFreeNo + 1] = nTP;
	m_nMemHP[nFreeNo + 1] = nHP;
	m_nMemMHP[nFreeNo + 1] = nMHP;
...
int		KTeam::GetMemberInfo(KUiPlayerItem *pList, int nCount)
{
	...
	pList[nNum].m_Level = m_nMemLevel[0]; //Them vao co level dau tien
	pList[nNum].m_TP = m_nMemTP[0];
	pList[nNum].m_SM = m_nMemSM[0];
	pList[nNum].m_HP = m_nMemHP[0]; ////
	pList[nNum].m_MHP = m_nMemMHP[0]; ////
	...
	for (i = 0; i < MAX_TEAM_MEMBER; i++)
	{
		...
		pList[nNum].m_Level = m_nMemLevel[i + 1];
		pList[nNum].m_TP = m_nMemTP[i + 1];////
		pList[nNum].m_SM = m_nMemSM[i + 1];////
		pList[nNum].m_HP = m_nMemHP[i + 1];////
		pList[nNum].m_MHP = m_nMemMHP[i + 1];////
