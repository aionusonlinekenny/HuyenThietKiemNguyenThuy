...
function GM_LearnSkill(nSel,nLevel)
	...
	local tbMessage = {
			"<color=green>Chóc mõng b¹n ®· häc ®­îc kü n¨ng "..nLevel.." "..Faction.TFACTION_INFO[nFactionID].sFactionName..".",
...
function GM_ChangeCamp()
	local sInfo = "<color=red>HÖ Thèng<color>: Xin mêi<color=green> "..GetName().." <color>lùa chän!"
	local tbSay = {
			"ChÝnh ph¸i./GM_ChangeCamp_1#1",
			"Tµ ph¸i./GM_ChangeCamp_1#2",
			"Trung lËp./GM_ChangeCamp_1#3",
			"S¸t thñ./GM_ChangeCamp_1#4",
